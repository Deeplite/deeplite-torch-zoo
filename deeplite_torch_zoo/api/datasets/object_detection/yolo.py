...
from deeplite_torch_zoo.src.object_detection.datasets.coco import (
    CocoDetectionBoundingBox,
)
from deeplite_torch_zoo.src.object_detection.datasets.coco.transforms import (
    default_transform_fn,
    random_transform_fn,
)
from deeplite_torch_zoo.src.object_detection.datasets.coco_config import (
    COCO_DATA_CATEGORIES,
    COCO_MISSING_IDS,
)
from deeplite_torch_zoo.src.object_detection.datasets.lisa import LISA
from deeplite_torch_zoo.src.object_detection.datasets.voc import VocDataset
from deeplite_torch_zoo.src.object_detection.datasets.voc_utils import (
...
def make_dataset_wrapper(wrapper_name, num_classes, img_size, dataset_create_fn):
    def wrapper_func(
        ...
        **kwargs,
        ...
        train_dataset, test_dataset = dataset_create_fn(
            data_root, num_classes, img_size
        )
...
def create_coco_datasets(data_root, num_classes, img_size, subsample_categories=None):
    ...
    if subsample_categories is not None:
        ...
        category_indices = [
            COCO_DATA_CATEGORIES["CLASSES"].index(cat) + 1 for cat in categories
        ]
        missing_ids = [
            category
            for category in list(range(1, 92))
    ...
    test_dataset = CocoDetectionBoundingBox(
        ...
        num_classes=num_classes,
        img_size=img_size,
        transform=default_transform_fn,
        classes=categories,
        category=category_indices,
        missing_ids=missing_ids,
    )
...
def create_lisa_datasets(data_root, num_classes, img_size):
    return LISA(data_root, _set="train", img_size=img_size), LISA(
        data_root, _set="valid", img_size=img_size
    )
...
def create_voc_format_datasets(data_root, num_classes, img_size):
    return create_voc_datasets(
        data_root, num_classes, img_size, standard_voc_format=False
    )


def create_car_detection_datasets(data_root, num_classes, img_size):
    """Part of COCO containing only the 'car' class"""
    return create_coco_datasets(
        data_root, num_classes, img_size, subsample_categories=['car']
    )
