...
def _mobilenetv2(arch, pretrained=False, num_classes=100, device='cuda'):
...
def mobilenet_v2_cifar100(
    pretrained=False, num_classes=100, device='cuda'
):
    return _mobilenetv2(
        "mobilenet_v2", pretrained, num_classes=num_classes, device=device
    )
