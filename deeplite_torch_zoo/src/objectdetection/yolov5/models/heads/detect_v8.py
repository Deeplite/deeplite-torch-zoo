...
def make_anchors(feats, strides, grid_cell_offset=0.5):
    ...
    for i, stride in enumerate(strides):
        ...
        sy, sx = torch.meshgrid(sy, sx, indexing='ij') if check_version(torch.__version__, '1.10.0') \
...
class DetectV8(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), act='relu'):  # detection layer
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3, act=act), Conv(c2, c2, 3, act=act), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = nn.ModuleList(nn.Sequential(Conv(x, c3, 3, act=act), Conv(c3, c3, 3, act=act), nn.Conv2d(c3, self.nc, 1)) for x in ch)
