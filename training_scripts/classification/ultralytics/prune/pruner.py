...
def get_layers_byname(model, layers={}, pre_name='', names=()):
    for name, layer in model.named_children():
        new_name = '.'.join([pre_name, name]) if pre_name != '' else name
        if len(list(layer.named_children())) > 0:
            get_layers_byname(layer, layers, new_name, names)
        else:
            if new_name in names:
                layers[new_name] = layer
    return layers

def get_layers_bytype(model, layers={}, pre_name='', get_conv=True, get_bn=False):
    for name, layer in model.named_children():
        new_name = '.'.join([pre_name, name]) if pre_name != '' else name
        if len(list(layer.named_children())) > 0:
            get_layers_bytype(layer, layers, new_name, get_conv, get_bn)
        else:
            get_layer = False
...
class Pruner(ABC):
    def __init__(self, model, config):
        self.model = model
        if hasattr(config, 'layers_toprune'):
            self.named_modules = get_layers_byname(
                    model, names=config.layers_toprune.keys())
        else:
            self.named_modules = get_layers_bytype(
                    model, get_conv=True, get_bn=False)
        self.layers_sparsity = config.layers_toprune
        self.scheduler = ConstantScheduler(
                begin_step=config.prune_start,
                frequency=config.prune_freq,
                dtype=float,
                initial_value=config.prune_init_value, # for now now the same across layers
        )
        if config.prune_method == 'ln_structured':
            self.prune_foo = ln_structured
            self.n = config.prune_n # n-order of norm
            self.dim = config.prune_dim # out channels
        else:
            raise NotImplementedError
        # self._unpruned_model = deepcopy(model)

    def step(self, global_step):
        if self.scheduler.should_do(global_step) is not True: # update masks
            return False
        else:
            # self.sparsity = self.scheduler.get_value(global_step)
            for name, module in self.named_modules.items():
                p = 1 # to fix with scheduler
                sparsity = self.layers_sparsity[name] * p
                self.prune_foo(module, 'weight', sparsity, self.n, self.dim)
                #if module.bias is not None: # TO-Fix for structured
                #    self.prune_foo(module, 'bias', sparsity, self.n, self.dim)
        return True

    def reset_mask(self):
        for name, module in self.named_modules.items():
            module.weight_mask.data.zero_().add_(1)
        return
    ...
    def reset_weights(self):
        for name, module in self.named_modules.items():
            ...
            #    module.bias.data.copy_(module.bias_orig.data)
        return

    def clean(self):
        for name, module in self.named_modules.items():
            module = remove(module, name='weight')
            #if module.bias is not None:
            # module = remove(module, name='bias')
        return
