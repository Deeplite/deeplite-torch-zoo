...
def train(opt, device):
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        is_prune_epoch = pruner.step(epoch)
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            # NOW weights are iotiized directly from sparse tensors
            # # Copy orig_weight and mask=1 before update
            # if is_prune_epoch:
        ...
        # Check sparsity (mask is ok, but w are dense) >>
        sparsity = {}
        for n, m in pruner.named_modules.items():
            sparsity[n] = float(torch.sum(m.weight == 0)) / float(
                    m.weight.nelement())
            # print("  * {:20s} >> {:.2f}".format(n, sparsity[n] * 100))
        # # Copy orig_weight and mask=1 before update
        if is_prune_epoch:
            #pruner.reset_mask()
            pruner.clean()
            print("     [x] Pruned Layers: {}".format(sparsity))
...
def parse_opt(known=False):
    ...
    # prune args
    parser.add_argument('--prune_method', type=str, default='ln_structured')
    parser.add_argument('--prune_dim', type=int, default=0)
    parser.add_argument('--prune_n', type=int, default=2)
    parser.add_argument('--prune_start', type=int, default=2)
    parser.add_argument('--prune_end', type=int, default=10)
    parser.add_argument('--prune_freq', type=int, default=2)
    parser.add_argument('--prune_init_value', type=int, default=1)
